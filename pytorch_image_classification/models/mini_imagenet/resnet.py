...
class Network(nn.Module):
    def __init__(self, config):
        ...
        initial_channels = model_config.initial_channels    # 64
        ...
        n_channels = [
            initial_channels,   # 64
            initial_channels * 2,   # 128
            initial_channels * 4,   # 256
            initial_channels * 8    # 512
        ]

        self.in_channel = initial_channels

        self.conv = nn.Conv2d(config.dataset.n_channels,    # 3: rgb
                              n_channels[0],    # 64
                              kernel_size=7,
                              stride=2,
                              padding=3,
                              bias=False)
        self.bn = nn.BatchNorm2d(initial_channels)  # 64
    ...
    def _make_stage(self, channel, n_blocks, block, stride):
        stage = nn.Sequential()
        downsample = None
        ...
        for index in range(n_blocks):
            ...
            if index == 0:
                stage.add_module(
                    block_name, block(self.in_channel, 
                                      channel,
                                      stride=stride,
                ...
                stage.add_module(block_name,
                                 block(self.in_channel, channel, stride=1))
        return stage
